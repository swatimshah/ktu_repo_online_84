...
combinedData = numpy.empty([0, 768])
...
for i in range (199):
	...
	pca_target = PCA(n_components=12)
	pca_target.fit(eegData)
	print(pca_target.components_.shape)
	combinedData = numpy.append(combinedData, pca_target.components_.flatten().reshape(1, 768), axis=0)
